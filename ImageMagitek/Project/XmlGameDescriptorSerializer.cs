        ...
        public IDictionary<string, ProjectResourceBase> DeserializeProject(string fileName, string baseDirectory)
        {
            ...
            /*foreach (XElement node in projectNode.Elements())
            {
                if (node.Name == "folder")
                {
                    ...
                    resourceTree.Add(folder.Name, folder);
                }
                else if (node.Name == "datafile")
                {
                    var df = DeserializeDataFile(node).ToDataFile();
                    resourceTree.Add(df.Name, df);
                }
                else if (node.Name == "palette")
                {
                    var pal = DeserializePalette(node).ToPalette();
                    //pal.LazyLoadPalette(dataFileKey, address, format, zeroIndexTransparent, entries);
        ...
        private IEnumerable<ProjectResourceBase> DeserializeChildren(XElement element)
        {
            foreach (XElement node in element.Elements())
            {
                if (node.Name == "folder")
                {
                    ...
                    {
                        var childResources = DeserializeChildren(child);
                        foreach (var childResource in childResources)
                            folder.ChildResources.Add(childResource.Name, childResource);
                    }*/
