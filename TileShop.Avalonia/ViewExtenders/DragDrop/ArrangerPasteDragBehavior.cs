...
namespace TileShop.AvaloniaUI.Behaviors;
public class ArrangerPasteDragBehavior : Behavior<Control>
{
    ...
    public static readonly StyledProperty<Type?> DataTypeProperty =
        AvaloniaProperty.Register<ArrangerPasteDragBehavior, Type?>(nameof(DataType));

    /// <summary>
    /// 
    /// </summary>
    public static readonly StyledProperty<IDragHandler?> HandlerProperty =
        AvaloniaProperty.Register<ArrangerPasteDragBehavior, IDragHandler?>(nameof(Handler));
    ...
    private async Task DoDragDrop(PointerEventArgs triggerEvent, object? value)
    {
        ...
        await Avalonia.Input.DragDrop.DoDragDrop(triggerEvent, data, effect);
    }
    ...
    private void AssociatedObject_PointerPressed(object? sender, PointerPressedEventArgs e)
    {
        ...
        {
            var (c1, c2, c3, c4) = (e.Source is IControl, 
                AssociatedObject?.DataContext == ((IControl)e.Source).DataContext,
                DataType is not null,
                DataType.IsAssignableFrom(((IControl)e.Source).DataContext?.GetType()));
