...
This is a modified version of PixiEditor.ColorPicker because the original does not size correctly when using a styling library.

I created DirectColorPicker for TileShop's use which required modifications to other core components, so PortableColorPicker etc are untested and probably broken.

Dark / Light mode switching at runtime with the library's theming does not seem possible without an extensive redesign of the styling in the library.
